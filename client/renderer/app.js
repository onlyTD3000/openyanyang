...
const 态 = {
  ...
  本地文件夹路径: '', // 用户在本地电脑上选的输出目录
  编辑主机: 0, // 主机表单正在改哪一台，0 表示新建
  编辑项目: 0, // 项目面板正在改哪个项目，0 表示新建
  回看位: null, // 回看需求的光标，null 表示还没开始翻
  /* 用户点过停止。
     光靠 在生成 挡不住任务链：收尾() 会把 在生成 置回 false，紧接着
     冲回执队列() 就把排队的回执发出去，新一轮又起来了；已经在飞的卡片
     执行完回调 fillXxxResult 时也是同样情况。表现就是点了停止，界面
     过一会儿自己又动起来，连着冒出好几个「已暂停」气泡。
     所以要一个独立于 在生成 的意图标志：用户点停止时置 true，
     下次真正发消息（用户敲回车或选了新会话）时清掉。 */
  已停止: false,
  当前本地路径: ''
};
...
const 元 = {
  ...
  声试听: 取('声试听'),
  /* 代码仓面板。聊天区这次也要取到：打开代码仓时整块藏掉 */
  聊天区: 取('聊天区'),
  代码仓: 取('代码仓'), 仓项目名: 取('仓项目名'), 仓概况: 取('仓概况'),
  仓仅改动: 取('仓仅改动'), 仓拉取: 取('仓拉取'), 仓回传: 取('仓回传'),
  仓关闭: 取('仓关闭'), 仓提示: 取('仓提示'), 仓搜索: 取('仓搜索'),
  仓文件表: 取('仓文件表'), 仓右: 取('仓右'),
  仓上传: 取('仓上传'), 仓选文件: 取('仓选文件')
};
...
/**
 * 自愈：修复历史 bug 造成的本地会话坏 id。
 * 旧版 新建本地对话() 把服务端返回的 conv_id 误读成 结果.id（不存在），
 * 导致新建会话存进 localStorage 的 id 是 NaN，序列化后变成 null，
 * 点开这条会话会让 态.当前会话 变成 0，发消息被误判成"没选对话"。
 * 这里逐个找出坏 id 的会话，向服务端重新创建一条真实会话，
 * 把已有的 messages（本地缓存的历史文本）原样搬过去，不丢内容。
 * 只在启动时跑一次，正常情况下（id 都合法）直接跳过，不产生任何请求。
 */
async function 修复本地坏会话() {
  try {
    const 表 = 读本地项目();
    let 有改动 = false;
    for (const 项 of 表) {
      if (!项.cloud_project_id) continue;   // 还没建云端项目的本地项目，新建对话时会自己建，跳过
      // 老数据历史遗留：这个影子项目当时建的时候服务端还没有 hidden 字段，
      // 一直混在云端项目列表里，容易被用户误当成普通项目删掉。
      // 每次启动补一次隐藏标记，补过一次后 项.影子已隐藏 = true，不用每次都发请求。
      if (!项.影子已隐藏) {
        const 隐藏结果 = await 调('/api/project.php', { act: 'hide', id: 项.cloud_project_id, hidden: 1 });
        if (隐藏结果 && !隐藏结果.error) { 项.影子已隐藏 = true; 有改动 = true; }
      }
      const convs = 项.convs || [];
      for (const 会话 of convs) {
        const 坏 = 会话.id === null || 会话.id === undefined || Number.isNaN(Number(会话.id)) || Number(会话.id) === 0;
        if (!坏) continue;
        const 结果 = await 调('/api/conv.php', {
          act: 'new',
          project_id: 项.cloud_project_id,
          title: 会话.title || '新对话'
        });
        if (!结果 || 结果.error || !结果.conv_id) continue;   // 建不了就先留着，不强修
        会话.id = Number(结果.conv_id);
        有改动 = true;
      }
    }
    if (有改动) 写本地项目(表);
...
/* 重启后回到上次那个会话。
   以前登录完就停在空界面，模型选择这类会话级设置只在 打开会话()
   里才会从接口读回来——不打开会话，界面就一直是 HTML 里的默认值，
...
async function 载入项目() {
  ...
  // 本地项目的服务端影子项目只用于会话归属，不能显示在云端项目列表。
  // 即使旧接口尚未及时返回 hidden=1，也按本地保存的 cloud_project_id 过滤。
  const 本地项目表 = 读本地项目();
  const 本地影子项目 = new Set(
    本地项目表
      .map((p) => Number(p.cloud_project_id || 0))
      .filter((id) => id > 0)
  );
  const 可见项目表 = 项目表.filter((p) =>
    !本地影子项目.has(Number(p.id)) &&
    Number(p.hidden || 0) !== 1 &&
    p.project_type !== 'local'
  );
...
const 项目折叠键 = (类型) => 'project_collapsed_' + 类型 + '_' + (态.用户id || 0);
function 读项目折叠状态(类型) {
  try {
    const 值 = JSON.parse(localStorage.getItem(项目折叠键(类型)) || '{}');
    return 值 && typeof 值 === 'object' ? 值 : {};
  } catch (_) {
    return {};
  }
}
function 写项目折叠状态(类型, id, 折叠) {
  try {
    const 状态 = 读项目折叠状态(类型);
    const 键值 = String(id);
    if (折叠) 状态[键值] = 1;
    else delete 状态[键值];
    localStorage.setItem(项目折叠键(类型), JSON.stringify(状态));
  } catch (_) {}
}

/* 本地项目的展开状态。
   跟上面那套刻意相反：存的是「哪些项目展开着」，不是「哪些折叠着」。
   因为要求打开软件默认全收起——存折叠集合的话，没记录的项目
   （新加的、换账号的）会默认展开，正好反了。
   用sessionStorage：它随进程结束自动清空，下次启动天然是空集合，
   「刚打开就默认关闭」不用另写清空逻辑。运行期间重绘读同一份，
   展开状态保得住。 */
const 本地展开键 = () => 'local_project_open_' + (态.用户id || 0);
function 读本地展开() {
  try {
const 值 = JSON.parse(sessionStorage.getItem(本地展开键()) || '{}');
    return 值 && typeof 值 === 'object' ? 值 : {};
  } catch (_) {
    return {};
  }
}
function 写本地展开(id, 展开) {
  try {
    const 状态 = 读本地展开();
    const 键值 = String(id);
    if (展开)状态[键值] = 1;
    else delete 状态[键值];
    sessionStorage.setItem(本地展开键(), JSON.stringify(状态));
  } catch (_) {}
}
...
function 画侧栏() {
  ...
  const 云端折叠 = 读项目折叠状态('cloud');
  态.项目.forEach((p) => {
    const li = 建元素('li');
    const 头 = 建元素('div', '项目名');
    头.appendChild(建元素('span', '', p.name));
    头.appendChild(建元素('span', '项目类型 云端标签', '云端'));
    ...
    // 代码仓入口用文件夹图标，和同排的 ＋ ⚙ ✕ 一样是单个符号，宽度才齐。
    // 用内联 SVG 不用 emoji：📁 在不同 Windows 版本上渲染不一致，有的机器
    // 出来是彩色方块，跟旁边几个单色符号放一起很突兀。SVG 跟着 currentColor 走，
    // hover 变色也和其它按钮同步。
    const 代码钮 = 建元素('button', '项目钮 图标钮');
    代码钮.innerHTML = '<svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">'
      + '<path d="M1.5 3.5h4l1.2 1.6h6.8a1 1 0 0 1 1 1v6.4a1 1 0 0 1-1 1H1.5'
      + 'a1 1 0 0 1-1-1V4.5a1 1 0 0 1 1-1z" fill="none" stroke="currentColor"'
      + ' stroke-width="1.3" stroke-linejoin="round"/></svg>';
    代码钮.setAttribute('aria-label', '打开项目代码仓');
...
function 仓提示(文本, 类型) {
  if (!元.仓提示) return;
  元.仓提示.textContent = 文本 || '';
  元.仓提示.className = '仓提示' + (类型 ? ' ' + 类型 : '');
}
function 仓参数(项目id, 额外) {
  return Object.assign({ act: 'info', project_id: Number(项目id) || 0 }, 额外 || {});
}
function 仓大小(字节) {
  const n = Number(字节) || 0;
...
function 仓状态标记(文件) {
  if (文件.state === 'new') return '新';
  if (文件.state === 'edited') return '改';
  return '';
}
function 仓画文件表() {
  if (!元.仓文件表) return;
  const 关键词 = (元.仓搜索 && 元.仓搜索.value || '').trim().toLowerCase();
  元.仓文件表.textContent = '';
  const 列表 = 仓文件.filter((文件) =>
    !关键词 || String(文件.path || '').toLowerCase().includes(关键词)
  );
  if (!列表.length) {
    元.仓文件表.appendChild(建元素('li', '空态', 关键词 ? '没有匹配的文件' : '代码仓为空'));
    return;
  }
  列表.forEach((文件) => {
    const 项 = 建元素('li', '仓文件项' + (文件.path === 仓当前路径 ? ' 选中' : ''));
    const 标记 = 建元素('span', '仓标记' + (文件.state === 'edited' ? ' 改' : '') + (文件.state === 'new' ? ' 新' : ''), 仓状态标记(文件));
    const 路径 = 建元素('span', '仓路径', 文件.path);
    项.appendChild(标记);
    项.appendChild(路径);
    项.title = String(文件.path || '');
    项.addEventListener('click', () => 读代码文件(文件));
    元.仓文件表.appendChild(项);
  });
}
function 仓按钮(文字, 类名, 标题) {
  const b = 建元素('button', 类名 || '次按钮 小', 文字);
  b.type = 'button';
  if (标题) b.title = 标题;
  return b;
}
/* zip 的右侧面板。上传是整包完整入仓，展开与否由用户在这里决定。
   原包解压后仍留在仓里——代码仓目前没有删除动作，不在这儿顺手加。 */
function 仓画解压面板(文件) {
  元.仓右.textContent = '';
  const 头 = 建元素('div', '仓文件头');
  头.appendChild(建元素('span', '仓文件路径', 文件.path));
  头.appendChild(建元素('span', '仓元信息',
    (文件.size_text || 仓大小(文件.size)) + ' · 压缩包'));
  const 解压钮 = 仓按钮('解压到当前目录', '主按钮 小',
    '把包里的文件解出来放到它所在的目录，原包保留');
  头.appendChild(解压钮);
  const 说明 = 建元素('div', '仓不可编辑',
    '压缩包已完整收进本地副本。点「解压」把里面的文件展开到 '
    + (String(文件.path).includes('/')
        ? String(文件.path).slice(0, String(文件.path).lastIndexOf('/'))
        : '仓根')
    + '，原包会保留。解压后的改动同样需要点顶栏「回传」才会到服务器。');
  解压钮.addEventListener('click', async () => {
    解压钮.disabled = true;
    仓提示('正在解压 ' + 文件.path + '…');
    const 结果 = await 调('/api/repo.php', {
      act: 'extract',
      project_id: 仓项目.id,
      path: 文件.path,
      full: '1'
    ...
    解压钮.disabled = false;
    if (!结果 || 结果.error) {
      仓提示('解压失败：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
      return;
    }
    await 仓刷新概况();
    await 仓刷新文件();
    仓提示(结果.msg || '解压完成', '好');
    // 跳过的条目单独列出来，不然用户只看到「解出 N 个」，
    // 不知道有几个因为路径不合法或超限被丢掉了
    const 跳 = (结果.skipped || []);
    if (跳.length) {
      const 详 = 建元素('div', '仓不可编辑',
        '跳过 ' + 跳.length + ' 个条目：\n' + 跳.join('\n'));
      详.style.whiteSpace = 'pre-wrap';
      元.仓右.appendChild(详);
    }
  });
  元.仓右.appendChild(头);
...
function 仓画编辑器(文件, 正文) {
  元.仓右.textContent = '';
  const 头 = 建元素('div', '仓文件头');
  const 路径 = 建元素('span', '仓文件路径', 文件.path);
  const 信息 = 建元素('span', '仓元信息',
    (文件.size_text || 仓大小(文件.size)) + ' · ' + (文件.state === 'clean' ? '未修改' : 文件.state));
  const 保存 = 仓按钮('保存本地改动', '主按钮 小', '只保存到客户端本地副本，不会立即回传服务器');
  头.appendChild(路径);
  头.appendChild(信息);
  头.appendChild(保存);
  const 编辑 = document.createElement('textarea');
  编辑.className = '仓编辑';
  编辑.spellcheck = false;
  编辑.value = 正文;
  编辑.dataset.path = 文件.path;
  保存.addEventListener('click', async () => {
    ...
    保存.disabled = false;
    if (!结果 || 结果.error) {
      仓提示('保存失败：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
      return;
    }
    文件.size = Number(结果.size || new TextEncoder().encode(编辑.value).length);
    文件.size_text = 结果.size_text || 仓大小(文件.size);
    文件.state = 'edited';
    文件.ver = Number(结果.ver || 文件.ver || 0);
    仓画文件表();
    仓提示('已保存到本地副本，尚未回传服务器', '好');
    仓刷新概况();
  });
  元.仓右.appendChild(头);
  元.仓右.appendChild(编辑);
...
async function 读代码文件(文件) {
  if (!文件 || !文件.path) return;
  仓当前路径 = 文件.path;
  仓画文件表();
  if (Number(文件.is_text) !== 1) {
    元.仓右.textContent = '';
    // zip 单独处理：它进仓时是整包原样收下的，这里给个解压入口把它展开。
    // 解出来的文件默认落在包所在的同级目录，跟本地双击解压的直觉一致。
    if (/\.zip$/i.test(String(文件.path || ''))) {
      仓画解压面板(文件);
      return;
    }
    元.仓右.appendChild(建元素('div', '仓不可编辑', '这是二进制或不可编辑文件，代码仓只支持文本文件。'));
    return;
  }
  元.仓右.textContent = '';
  元.仓右.appendChild(建元素('div', '仓空态', '正在读取 ' + 文件.path + '…'));
  const 结果 = await 调('/api/repo.php', 仓参数(仓项目.id, { act: 'read', path: 文件.path }), 'GET');
  if (!结果 || 结果.error) {
    仓提示('读取失败：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
    元.仓右.textContent = '';
    元.仓右.appendChild(建元素('div', '仓空态', '文件读取失败'));
    return;
  }
  仓画编辑器(文件, String(结果.text || ''));
}
async function 仓刷新概况() {
  if (!仓项目 || !元.仓概况) return;
  const 结果 = await 调('/api/repo.php', 仓参数(仓项目.id), 'GET');
  if (!结果 || 结果.error) {
    元.仓概况.textContent = '';
    return;
  }
  const r = 结果.repo || {};
  元.仓概况.textContent = (r.file_count || 0) + ' 个文件 · ' +
    (r.bytes_text || 仓大小(r.bytes)) +
    (Number(r.dirty) > 0 ? ' · 未回传 ' + r.dirty : '') +
    (r.host ? ' · ' + r.host : '');
}
async function 仓刷新文件() {
  if (!仓项目) return;
  仓提示('正在加载文件清单…');
  const 结果 = await 调('/api/repo.php', 仓参数(仓项目.id, {
    act: 'tree',
    dirty: 元.仓仅改动 && 元.仓仅改动.checked ? 1 : 0
  }), 'GET');
  if (!结果 || 结果.error) {
    仓文件 = [];
    仓画文件表();
    仓提示('加载失败：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
    return;
  }
  仓文件 = Array.isArray(结果.list) ? 结果.list : [];
  仓画文件表();
  仓提示('已加载 ' + 仓文件.length + ' 个文件', '好');
}
async function 开代码仓(项目) {
  if (!项目 || !项目.id) return;
  仓项目 = 项目;
  仓当前路径 = '';
  态.当前项目 = Number(项目.id);
  if (元.仓项目名) 元.仓项目名.textContent = 项目.name || '代码仓';
  if (元.聊天区) 元.聊天区.classList.add('隐藏');
  if (元.代码仓) 元.代码仓.classList.remove('隐藏');
  if (元.仓右) {
    元.仓右.textContent = '';
    元.仓右.appendChild(建元素('div', '仓空态', '正在打开代码仓…'));
  }
  await 仓刷新概况();
  await 仓刷新文件();
}
function 关代码仓() {
  ...
  仓文件 = [];
  仓当前路径 = '';
  if (元.代码仓) 元.代码仓.classList.add('隐藏');
  if (元.聊天区) 元.聊天区.classList.remove('隐藏');
}
if (元.仓关闭) 元.仓关闭.addEventListener('click', 关代码仓);
if (元.仓搜索) 元.仓搜索.addEventListener('input', 仓画文件表);
if (元.仓仅改动) 元.仓仅改动.addEventListener('change', 仓刷新文件);
/* 代码仓上传。走主进程的通用 multipart 通道：
   渲染层碰不到网络，文件读成 Uint8Array 经 IPC 传过去。
   一次选多个就挨个传，中间任何一个失败都停下来报出来——
   继续往下传只会让用户搞不清仓里到底进了哪几个。 */
const 能传仓文件 = !!(window.后端 && window.后端.传文件);
if (元.仓上传 && 元.仓选文件) {
  if (!能传仓文件) {
    元.仓上传.disabled = true;
    元.仓上传.title = '当前客户端版本不支持上传，请更新';
  } else {
    元.仓上传.addEventListener('click', () => 元.仓选文件.click());
    元.仓选文件.addEventListener('change', async () => {
      const 文件们 = Array.from(元.仓选文件.files || []);
      元.仓选文件.value = '';   // 清掉，不然选同一个文件第二次不触发 change
      if (!文件们.length || !仓项目) return;
      元.仓上传.disabled = true;
      let 成 = 0;
      for (const f of 文件们) {
        仓提示('正在上传 ' + f.name + '…（' + (成 + 1) + '/' + 文件们.length + '）');
        let 字节;
        try {
          字节 = new Uint8Array(await f.arrayBuffer());
        } catch (err) {
          仓提示('读取本地文件失败：' + f.name, '坏');
          break;
        }
        const 结果 = await window.后端.传文件('/api/repo.php', {
          name: f.name,
          ...
          act: 'upload',
          project_id: 仓项目.id,
          // 一律完整入仓，不在上传时解压。要展开就上传后点文件里的「解压」
          mode: 'keep',
          full: '1'
        }, 态.密钥);
        if (!结果 || 结果.error) {
          仓提示('上传失败（' + f.name + '）：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
          break;
        }
        成++;
        仓提示(结果.msg || (f.name + ' 已上传'), '好');
      }
      元.仓上传.disabled = false;
      仓当前路径 = '';
      await 仓刷新概况();
      await 仓刷新文件();
      if (成 === 文件们.length && 文件们.length > 1) {
        仓提示('全部 ' + 成 + ' 个文件已进本地副本，改动还未回传', '好');
      }
    });
  }
}
if (元.仓拉取) 元.仓拉取.addEventListener('click', async () => {
  if (!仓项目) return;
  元.仓拉取.disabled = true;
  仓提示('正在从服务器拉取…');
  const 结果 = await 调('/api/repo.php', {
  ...
  元.仓拉取.disabled = false;
  if (!结果 || 结果.error) {
    仓提示('拉取失败：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
    return;
  }
  仓当前路径 = '';
  await 仓刷新概况();
...
if (元.仓回传) 元.仓回传.addEventListener('click', async () => {
  if (!仓项目) return;
  元.仓回传.disabled = true;
  仓提示('正在回传本地改动…');
  const 结果 = await 调('/api/repo.php', {
  ...
  元.仓回传.disabled = false;
  if (!结果 || 结果.error) {
    仓提示('回传失败：' + ((结果 && 结果.error) || '服务端返回异常'), '坏');
    return;
  }
  await 仓刷新概况();
  await 仓刷新文件();
...
/* ---- 本地项目 ---- */
const 本地存储键 = () => 'local_projects_' + (态.用户id || 0);
function 读本地项目() {
  try { return JSON.parse(localStorage.getItem(本地存储键()) || '[]'); }
  catch (_) { return []; }
}
function 写本地项目(列表) {
  try { localStorage.setItem(本地存储键(), JSON.stringify(列表)); }
  catch (_) {}
}
/* 侧栏重绘或登录流程刷新项目列表后，把 window.当前项目 的本地目录信息
   补齐。载入项目() 只重建 态.项目（云端项目），不会碰 window.当前项目，
   如果当前打开的正是一个本地会话，这里要把它的 local_dir 找回来，
   否则文件卡片拿到的 window.当前项目.local_dir 会是旧值或空值，
   写入本地文件就会报"本地项目未指定文件夹路径"。 */
function 恢复当前本地上下文() {
  if (!window.当前项目 || window.当前项目.deploy_type !== 'local') return;
  const 表 = 读本地项目();
  const 项目 = 表.find((p) => Number(p.id) === Number(window.当前项目.id));
  if (!项目) return;
  window.当前项目 = {
    id: 项目.id,
    cloud_project_id: Number(项目.cloud_project_id || 0),
    name: 项目.name,
    deploy_type: 项目.deploy_type || 'local',
    local_dir: 项目.local_dir || ''
  };
}
/* 本地项目专用：往当前会话的 messages 数组里追加一条消息并落盘。
   只对 deploy_type === 'local' 的项目生效，云端项目消息由服务端 conv.php 管，
   不走这里，否则会把云端消息也误写进 localStorage。
   态.当前项目 在本地会话场景下存的是 cloud_project_id（见 打开本地会话），
   所以查找时要按 cloud_project_id 匹配，退回用 id 兼容还没生成云端 id 的情况。 */
function 存本地消息(角色, 文本) {
  try {
    if (!window.当前项目 || window.当前项目.deploy_type !== 'local') return;
    const 表 = 读本地项目();
    const 项 = 表.find(x => (x.cloud_project_id || x.id) === 态.当前项目);
    if (!项) return;
    const convs = 项.convs || [];
    const 会话 = convs.find(c => Number(c.id) === 态.当前会话);
    if (!会话) return;
    会话.messages = 会话.messages || [];
    会话.messages.push({ role: 角色, content: 文本, ts: Date.now() });
    写本地项目(表);
  } catch (_) {}   // 落盘失败不影响主流程，本地记录丢一条不至于打断发送
}
function 画本地项目侧栏() {
  const 列表 = 读本地项目();
  // 重绘前读展开状态，保证重绘后用户折叠的项目不会弹回来
  const 展开集 = 读本地展开();
  元.本地项目列表.textContent = '';
  if (!列表.length) {
    元.本地项目列表.appendChild(建元素('li', '空态', '还没有本地项目'));
    return;
  }
  列表.forEach((p) => {
    const li = 建元素('li');
    const 头 = 建元素('div', '项目名');
    头.appendChild(建元素('span', '', p.name));
    头.appendChild(建元素('span', '项目类型 本地标签', '本地'));
    const 操作行 = 建元素('span', '项目操作');
    const 新话钮 = 建元素('button', '项目钮', '＋');
    const 设置钮 = 建元素('button', '项目钮', '⚙');
    const 删钮 = 建元素('button', '项目钮 危险', '✕');
    新话钮.title = '在这个项目下新建对话';
    设置钮.title = '本地项目设置';
    删钮.title = '移除本地项目';
    新话钮.addEventListener('click', (ev) => {
    ...
    删钮.addEventListener('click', (ev) => {
      ev.stopPropagation();
      if (!window.confirm('移除对「' + p.name + '」的本地管理？\n不会删除文件夹里的文件。')) return;
      const 表 = 读本地项目().filter(x => x.id !== p.id);
      写本地项目(表);
      if (态.当前会话 && 态.当前项目 === p.id) {
        态.当前会话 = 0;
        态.当前项目 = 0;
        元.消息区.textContent = '';
      }
    ...
    convs.forEach((c) => {
      ...
      项.appendChild(标题);
      项.appendChild(操作);
      // dataset.projectId 要存跟 态.当前项目 同一套 id 体系（云端 project_id），
      // 不能存本地项目 id：打开本地会话() 把 态.当前项目 设成 cloud_project_id，
      // 这里存的若是本地 id，两边永远比不上，选中样式就一直加不上去。
      项.dataset.id = c.id;
      项.dataset.projectId = p.cloud_project_id || p.id;
      if (Number(c.id) === 态.当前会话 && Number(项.dataset.projectId) === 态.当前项目) 项.classList.add('选中');
      项.addEventListener('click', () => 打开本地会话(p, c));
      改名钮.addEventListener('click', (ev) => { ev.stopPropagation(); 改本地会话名(p, c, 项); });
      删除钮.addEventListener('click', (ev) => { ev.stopPropagation(); 删本地会话(p, c); });
      ul.appendChild(项);
    });
    // 默认收起：只有本次运行里被用户点开过的才展开
    if (!展开集[String(p.id)]) ul.classList.add('隐藏');
    li.appendChild(ul);

    头.addEventListener('click', () => {
      态.当前项目 = p.id;
      const 隐了 = ul.classList.toggle('隐藏');
      // 记下来，否则下次重绘（跑完命令刷侧栏）又恢复原样
      写本地展开(p.id, !隐了);
    });
    元.本地项目列表.appendChild(li);
  });
}
async function 新建本地对话(项目) {
  const 表 = 读本地项目();
  const 项 = 表.find(x => x.id === 项目.id);
  if (!项) return;

  // 确保本地项目有对应的云端 project_id
  if (!项.cloud_project_id) {
    const 创建结果 = await 调('/api/project.php', {
      act: 'save',
      id: 0,
      name: 项.name,
      intro: 项.intro || '',
    ...
    if (!创建结果 || 创建结果.error) {
      提示错误('创建云端项目失败：' + ((创建结果 && 创建结果.error) || '未知错误'));
      return;
    }
    项.cloud_project_id = Number(创建结果.id);
    写本地项目(表);
  }

  // 创建服务端真实会话，拿到真实 conv_id
  const 结果 = await 调('/api/conv.php', {
    act: 'new',
    project_id: 项.cloud_project_id,
    title: '新对话'
  ...
  if (!结果 || 结果.error) {
    提示错误('创建会话失败：' + ((结果 && 结果.error) || '未知错误'));
    return;
  }

  const 新会话 = {
    id: Number(结果.conv_id),
    title: 结果.title || '新对话',
    project_id: 项.cloud_project_id,
    messages: [],
  };
...
function 打开本地会话(项目, 会话) {
  if (态.在生成) return;
  态.已停止 = false;
  态.当前会话 = Number(会话.id);
  // 本地项目用云端 project_id 传给服务端，会话才能通过 project_of 校验
  const 真项目id = 项目.cloud_project_id || 项目.id;
  态.当前项目 = Number(真项目id);
  记住会话(态.当前会话, 态.当前项目);
  元.标题.textContent = 会话.title || '新对话';
  // 只更新侧栏里会话项的选中状态，不要重绘整个侧栏
  const 所有项 = 元.本地项目列表.querySelectorAll('.会话项');
  所有项.forEach(el => {
    const cid = Number(el.dataset.id);
    const pid = Number(el.dataset.projectId);
    if (cid === 态.当前会话 && pid === 态.当前项目) {
      el.classList.add('选中');
    } else {
      el.classList.remove('选中');
    }
  });

  // 让卡片脚本拿到完整项目信息（用于本地/云端分支）
  window.当前项目 = {
    id: 项目.id,
    name: 项目.name,
    deploy_type: 项目.deploy_type || 'local',
  ...
  const 消息们 = 会话.messages || [];
  画消息(消息们, 项目);
}
/* 删除本地会话。只存在于 localStorage 里，没有后端接口。 */
function 删本地会话(项目, 会话) {
  ...
  const 项 = 表.find(x => x.id === 项目.id);
  if (!项) return;
  项.convs = (项.convs || []).filter(c => c.id !== 会话.id);
  写本地项目(表);

  if (Number(会话.id) === 态.当前会话 && 态.当前项目 === 项目.id) {
    态.当前会话 = 0;
    态.当前项目 = 0;
    态.当前run = 0;
    态.在生成 = false;
...
/* 重命名本地会话。跟云端一样的行内输入框方式。 */
function 改本地会话名(项目, 会话, 项元素) {
  ...
  const 收 = (提交) => {
    ...
    if (!提交 || !新名 || 新名 === 原名) { 画本地项目侧栏(); return; }

    const 表 = 读本地项目();
    const 项 = 表.find(x => x.id === 项目.id);
    if (!项) return;
    const 会话们 = 项.convs || [];
    const c = 会话们.find(x => x.id === 会话.id);
    if (c) c.title = 新名;
    写本地项目(表);
  ...
  框.addEventListener("keydown", (ev) => {
    if (ev.key === "Enter") 收(true);
    if (ev.key === "Escape") 收(false);
  });
  框.addEventListener("blur", () => 收(true));
}
...
async function 打开会话(会话, 项目id) {
  ...
  /* 换会话就重新开始，把停止标志清掉。
     不清的话：上一个会话点过停止，切到别的会话后回执链仍被挡着，
     卡片自动执行也不工作，得手动发一条消息才恢复。 */
  态.已停止 = false;
  态.当前会话 = Number(会话.id);
  ...
  // 让卡片脚本拿到完整项目信息（用于本地/云端分支）
  var 项目对象 = (态.项目 || []).find(function(p){ return p.id === 态.当前项目; });
  window.当前项目 = 项目对象 ? {
    id: 项目对象.id,
    name: 项目对象.name,
    deploy_type: 'cloud',
    host_id: 项目对象.host_id,
    deploy_dir: 项目对象.deploy_dir
  } : null;
...
function 建上传入口() {
  ...
  选文件.type = 'file';
  选文件.accept = 'image/jpeg,image/png,image/gif,image/webp,.zip,.rar,.7z,.tar,.gz,.bz2,.xz,.tar.gz,.tar.bz2,.tar.xz';
  选文件.multiple = true;
  选文件.className = '隐藏';
...
/* 拖进来的文件能不能传。
   不能只看 f.type：Windows 上 zip 的 MIME 有时是空串、有时是
   application/x-zip-compressed，rar 和 7z 基本都是空的，只认 MIME 会把
   压缩包全过滤掉——这就是之前拖压缩包没反应的原因。
   所以图片仍按 MIME 判断（这个可靠），压缩包按扩展名判断。
   白名单跟服务端 api/upload.php 保持一致，真正的校验在服务端。 */
function 可上传(f) {
  if (!f) return false;
  if (f.type && f.type.indexOf('image/') === 0) return true;
  return /\.(zip|rar|7z|tar|gz|bz2|xz|tar\.gz|tar\.bz2|tar\.xz)$/i.test(f.name || '');
}
...
async function 传一张(文件) {
  ...
  态.待发图.push({
    ...
    是压缩包: r.is_archive || false,
    文件名: r.name || 文件.name || '未命名'
  });
  画待发图();
}
...
function 画待发图() {
  ...
  态.待发图.forEach((图, i) => {
    ...
    x.addEventListener('click', () => {
      if (!图.是压缩包) URL.revokeObjectURL(图.预览);
      态.待发图.splice(i, 1);
      画待发图();
    });
    格.appendChild(x);
...
async function 发送(外部文本, 工具类) {
  ...
  /* 用户主动发言，解除暂停状态。
     只认非回执：回执是机器发的，暂停后排队的回执不该把标志清掉，
     否则它自己就把自己放行了。
     清除标志后，冲回执队列() 会自动把积压的回执逐个发出去。 */
  if (!是回执) {
    态.已停止 = false;
    // 立即触发一次队列清空，把暂停期间积压的回执发出去
    setTimeout(() => 冲回执队列(), 0);
  }
...
/** 统一入口：能发就发，不能发就排队。 */
function 交回执(文, 类) {
  /* 点暂停时可能已有卡片在执行（SSH 命令、HTTP 请求都是异步的），
     暂停只断了对话的 HTTP 连接，管不住这些在路上的回调。
     它们执行完走到这里，回执仍然入队保留，等用户恢复对话时再发。 */
  if (态.在生成 || 态.已停止) { 回执队列.push({ 文: 文, 类: 类 }); return; }
  发送(文, 类);
}
...
/* web_card.js：网页正文。不在这里截断——服务端已按 limit 截过并标了 truncated，
   这边再截一刀，AI 拿到残缺内容还以为是全文。 */
...
function 收尾(错误文本) {
  ...
  if (节点) {
    ...
    // 不渲染的话 ssh-exec 之类的代码块只是一段文字，没有卡片可执行，
       ...
       渲染气泡(节点);
       // 本地项目：助手消息存入 localStorage
       const 助手正文 = (节点.文 && 节点.文.textContent) || '';
       存本地消息('assistant', 助手正文);
     }
     当前节点 = null;
 ...
 */
function 自动执行卡片(节点) {
  if (态.已停止) return;   // 停止后不再拉起新的卡片执行
  const 容器 = 节点 && 节点.泡;
  if (!容器) return;
  ['sshAutoRun', 'repoAutoRun', 'sftpAutoRun', 'wsAutoRun', 'pptAutoRun',
   'webAutoRun'].forEach(function (名) {
...
window.后端.收流((流号, 事件, 数据) => {
  ...
  if (事件 === '工具') {
    // 工具执行完成：把 tool_result 事件渲染成可折叠卡片。
    // 主进程把 tool_result 映射成了「工具」，数据仍是 {calls, results}。
    // 模型可能分多轮连续调用工具，后端每轮发一个事件只带当轮的调用，
    // 所以这里按 id 去重合并，后到的卡片追加到气泡里而不是覆盖前面的。
    if (当前节点 && 数据 && Array.isArray(数据.calls)) {
      const 泡 = 当前节点.泡;
      const 结果们 = Array.isArray(数据.results) ? 数据.results : [];
      (数据.calls || []).forEach((调用) => {
        const 调用id = 调用.id || '';
        // 同一 id 已渲染过就跳过，避免重复卡片
        if (泡.querySelector('.tool-card[data-tool-id="' + 调用id + '"]')) return;
        const 工具名 = (调用.function && 调用.function.name) || '';
        let 参数原文 = (调用.function && 调用.function.arguments) || '{}';
        // 通过 tool_call_id 匹配结果，而不是按索引
        let 结果 = '';
        (结果们 || []).forEach((r) => {
          if (r && r.tool_call_id === 调用id) 结果 = r.content || '';
        });
        // 参数 JSON 格式化显示，方便查看命令内容
        let 参数文本 = 参数原文;
        try { 参数文本 = JSON.stringify(JSON.parse(参数原文), null, 2); } catch (e) { /* 保持原样 */ }

        const 卡 = 建元素('details', 'tool-card');
        if (调用id) 卡.dataset.toolId = 调用id;
        const 头 = 建元素('summary', 'tool-card-head');
        头.appendChild(建元素('span', 'tool-call-ico', '▸'));
        头.appendChild(建元素('span', 'tool-card-title', '工具调用: ' + 工具名));
        头.appendChild(建元素('span', 'tool-card-state good', '执行完成'));
        卡.appendChild(头);
        const 体 = 建元素('div', 'tool-card-body');
        体.appendChild(建元素('div', 'tool-card-label', '参数'));
        const 参码 = 建元素('pre', 'tool-card-args');
        参码.textContent = 参数文本;
        体.appendChild(参码);
        if (结果) {
          体.appendChild(建元素('div', 'tool-card-label', '执行结果'));
          const 果码 = 建元素('pre', 'tool-card-out');
          果码.textContent = 结果;
          体.appendChild(果码);
        }
        卡.appendChild(体);
        // 工具卡片插在正文前面：先看到工具干了什么，再看到 AI 的总结
        泡.insertBefore(卡, 当前节点.文);
      });
      元.消息区.scrollTop = 元.消息区.scrollHeight;
    }
    return;
...
元.停止.addEventListener('click', async () => {
  /* 先置标志再做别的：收尾() 里会调 冲回执队列()，
     标志没先立起来的话那批回执照样发得出去，等于没停。 */
  态.已停止 = true;
  清看门狗();
...
async function 开项目面板(项目) {
  项目 = 项目 || {};
  态.编辑项目 = 项目.id || 0;
  const 是本地 = 项目.deploy_type === 'local';
  元.项目面板标题.textContent = 项目.id ? '编辑项目' : '新建项目';
  元.项目提示.textContent = '';
  元.p_name.value = 项目.name || '';
  元.p_intro.value = 项目.intro || '';
  元.p_stack.value = 项目.stack || '';
  元.p_type.value = 是本地 ? 'local' : 'cloud';
  切部署区(是本地 ? 'local' : 'cloud');
  if (是本地) {
    元.本地路径.textContent = 项目.local_dir || '';
  } else {
...
function 切部署区(类型) {
  const 云 = 类型 === 'cloud';
  元.云端区.style.display = 云 ? '' : 'none';
  元.本地区.style.display = 云 ? 'none' : '';
}
/** 保存项目。 */
async function 存项目() {
  ...
  const 部署位置 = 元.p_type.value;  /* cloud 或 local */
  // --------------- 云端：原逻辑 ---------------
  if (部署位置 === 'cloud') {
  ...
  // --------------- 本地 ---------------
  if (部署位置 === 'local') {
    const 目录 = 元.本地路径.textContent.trim();
    if (!目录) {
      元.项目提示.textContent = '请先选择本地文件夹';
      return;
    }
    元.项目提示.textContent = '保存中…';
    const 列表 = 读本地项目();
    const id = 态.编辑项目 || Date.now();
    const 旧 = 列表.find(p => p.id === id);
    if (旧) {
      旧.name = 名;
      旧.local_dir = 目录;
      旧.intro = 元.p_intro.value.trim();
      旧.stack = 元.p_stack.value.trim();
      旧.deploy_type = 'local';
    } else {
      // 新建本地项目时，也在服务端创建一个对应的隐藏项目
      const 云端结果 = await 调('/api/project.php', {
        act: 'save',
        id: 0,
        name: 名,
        intro: 元.p_intro.value.trim(),
      ...
      if (!云端结果 || 云端结果.error) {
        元.项目提示.textContent = '创建失败：' + ((云端结果 && 云端结果.error) || '未知错误');
        return;
      }
      列表.push({
        id,
        ...
        stack: 元.p_stack.value.trim(),
        deploy_type: 'local',
        cloud_project_id: Number(云端结果.id),
        convs: []
      });
    }
    写本地项目(列表);
...
function 绑项目面板() {
  ...
  // 部署位置切换
  元.p_type.addEventListener('change', () => {
    切部署区(元.p_type.value);
  });
  // 本地文件夹选择
  元.选本地文件夹.addEventListener('click', async () => {
    const 结果 = await window.后端.选择文件夹();
    if (结果.ok && 结果.path) {
      元.本地路径.textContent = 结果.path;
    }
  });
}
/* 「检查更新」按钮。
...
function 绑更新钮() {
  ...
  window.后端.收更新进度(({ 已下, 总长 }) => {
    if (!(总长 > 0)) { return; }
    // 夹到 0-100：content-length 缺失时总长会退化成后台登记的 size，
    // 两者不一致就可能算出 100 以上的数
    let 百分 = Math.floor(已下 / 总长 * 100);
    if (百分 < 0) { 百分 = 0; }
    if (百分 > 100) { 百分 = 100; }
    // 只许往前走。进度倒退一定是异常（多个流在写），显示上先稳住，
    // 真正的并发问题在主进程用锁解决
    if (百分 <= 上次百分) { return; }
    上次百分 = 百分;
    $钮.textContent = 百分 + '%';
...
/* ---- 本地文件夹功能 ---- */
function 绑本地文件夹() {
  const 钮 = 元.保存文件夹钮; if (!钮) return;
  钮.addEventListener('click', async () => {
    const r = await window.后端.选择文件夹();
    if (r && r.ok) {
      态.本地文件夹路径 = r.path;
      let 标 = document.getElementById('本地文件夹路径');
      if (!标) {
        标 = document.createElement('span');
        标.id = '本地文件夹路径';
        标.className = '标题栏文本';
        标.style.fontSize = '12px';
        标.style.marginLeft = '6px';
        钮.parentNode.insertBefore(标, 钮.nextSibling);
      }
      标.textContent = '已选: ' + r.path;
    }
  });
}
