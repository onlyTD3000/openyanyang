...
(function () {
  ...
  window.pptCard = function (raw) {
    ...
    return '<div class="repo-card ppt-card-op" id="' + id + '">' +
  ...
  function 执行(card) {
    ...
    return fetch('/api/ppt.php', { method: 'POST', body: fd, credentials: 'same-origin' })
      ...
      .then(function (r) {
        ...
        // 挂下载按钮。用 a 标签直接指向下载接口，不用 JS 触发，
        // 这样右键另存、复制链接都正常。
        var foot = card.querySelector('.repo-card-foot');
        if (foot && !foot.querySelector('.ppt-dl')) {
          var a = document.createElement('a');
          a.className = 'ppt-dl';
          a.href = r.url;
          a.textContent = '下载 ' + r.name + '（' + r.slides + ' 页 / ' + r.size_text + '）';
